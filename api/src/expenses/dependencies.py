...
from groups.service import get_member, is_member
...
async def get_expense_as_creator_or_admin(
    session: DbSession, authenticated_user: AuthenticatedUser, expense_id: Annotated[int, Path()]
) -> Expense:
    """
    Dependency that fetches an expense and verifies the user is either the creator
    or an admin of the expense's group.
    Returns 404 if expense doesn't exist or user is not a group member.
    Returns 403 if user is a member but neither the creator nor an admin.
    """
    expense = get_expense_by_id(session=session, expense_id=expense_id)
    member = get_member(session=session, group_id=expense.group_id, user_id=authenticated_user.id) if expense else None
    if not expense or not member:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Expense not found")
    if expense.created_by != authenticated_user.id and not member.is_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to modify this expense")
    return expense
...
ExpenseAsCreatorOrAdmin = Annotated[Expense, Depends(get_expense_as_creator_or_admin)]

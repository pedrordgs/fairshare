...
from expenses.dependencies import get_expense_as_creator, get_expense_as_creator_or_admin, get_expense_as_member
...
class TestGetExpenseAsCreatorOrAdmin:
    @pytest.mark.anyio
    async def test_returns_expense_for_creator(self, session) -> None:
        user = create_user(
            session=session, user_in=UserCreate(name="Test", email="test@example.com", password="password")
    ...
    async def test_returns_expense_for_group_admin(self, session) -> None:
        ...
        promote_member(session=session, group=group, user_id=admin.id)
        ...
        result = await get_expense_as_creator_or_admin(session=session, authenticated_user=admin, expense_id=expense.id)
        assert result.id == expense.id
    ...
    async def test_raises_403_for_non_creator_non_admin_member(self, session) -> None:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_expense_as_creator_or_admin(session=session, authenticated_user=member, expense_id=expense.id)
        assert exc_info.value.status_code == 403
        assert exc_info.value.detail == "Not authorized to modify this expense"
    ...
    async def test_raises_404_when_not_member(self, session) -> None:
        ...
        # Non-member sees 404 (not 403) to avoid leaking info
        with pytest.raises(HTTPException) as exc_info:
            await get_expense_as_creator_or_admin(session=session, authenticated_user=other_user, expense_id=expense.id)
        assert exc_info.value.status_code == 404
    ...
    async def test_raises_404_when_expense_not_found(self, session) -> None:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_expense_as_creator_or_admin(session=session, authenticated_user=user, expense_id=99999)
        assert exc_info.value.status_code == 404
